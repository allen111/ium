...
f.Show()
let imgWall = Image.FromFile("C:/Users/allen/Desktop/Wall.png")
  ...
  if grilled then
    for a in 0..Heigt-1 do (
            for b in 0..Width-1 do(
                match arr.[a,b] with
                    |0-> g.FillRectangle(Brushes.SteelBlue,b*20,a*20,19,19)
                    |1->g.FillRectangle(Brushes.Black,b*20,a*20,19,19)
                    |2->g.DrawImage(imgDoor,Rectangle(b*20,a*20,19,19))
                    |_->()
//                if arr.[a,b]=1 then
...
pnl.MouseUp.Add(fun e->
   ...
   if grilled && dragging then
        ...
        if tempY<Heigt && tempX<Width && tempY>=0 && tempX>=0 then
          match e.Button with
                |MouseButtons.Left->arr.[tempY,tempX]<-1
                |MouseButtons.Right->arr.[tempY,tempX]<-0
                |MouseButtons.Middle->arr.[tempY,tempX]<-2
                |_->() 
...
if grilled then
    ...
    let x=System.IO.File.WriteAllText(@"C:\Users\allen\Desktop\b1.txt",s)
